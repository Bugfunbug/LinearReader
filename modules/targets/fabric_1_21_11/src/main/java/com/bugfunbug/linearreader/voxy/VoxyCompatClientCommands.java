...
public final class VoxyCompatClientCommands {
    ...
    static void register(CommandDispatcher<FabricClientCommandSource> dispatcher) {
        var root = ClientCommandManager.literal("linearreader")
                .executes(ctx -> forwardToServer(ctx.getSource(), "linearreader"));

        if (FabricLoader.getInstance().isModLoaded("voxy")) {
            root.then(ClientCommandManager.literal("voxy-compat")
                    .executes(ctx -> executeStatus(ctx.getSource()))
                    .then(ClientCommandManager.literal("auto")
                            .executes(ctx -> executeAuto(ctx.getSource()))));
        }

        root.then(ClientCommandManager.argument("serverCommand", StringArgumentType.greedyString())
